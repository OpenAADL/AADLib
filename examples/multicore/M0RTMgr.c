...
void m0rtinit(initmsg *msg){
	printf("Mission Bus 0 Remote Terminal Initialization - Start\n");
	msg->InitializationComplete = TRUE;
	printf("Mission Bus 0 Remote Terminal  - Complete\n");
}

void m0rtmgr(initmsg msg, busmsg *RTtoBCMsg){
	if(msg.InitializationComplete){
		if(FirstTimeThrough){
			printf("Mission Bus 0 Remote Terminal  - Start\n");
		...
		if(MessageCounter == 0){
			...
			printf("RT0 Sending Message (%d) to BC\n",RTtoBCMsg->MsgQueue[0].Msg[1]);
		}
		else {
