...
float wp_distance=0.0;   /* Stores the distances from the current waypoint */
...
float wp_bearing=0.0;       /* Stores the bearing from the current waypoint */
...
extern void gps_backdoor (float yaw);

void flt_mgmt_simulation (ardupilot_software__gps_position_i pos, int* speed, int* angle)
{
  int altitude; 
   ...
   *angle = middle_yaw + PID_heading (compass_error (45, wp_bearing)); 
